...
const PROJECTS = [
  {
    ...
    // Extended sections for the case study
    sections:[
      {
        kind:"films",
    ...
    // Stat row for the featured spotlight
    stats:[
      ["Films delivered","03"],
      ["Posters drawn","18"],
      ["Merch SKUs","16"],
...
// ─── localStorage merge ──────────────────────────────────────────────
// The admin dashboard and inline editor both write to lc:projects in browser
// storage. Storage format: array of the full PROJECTS list (new format) — or
// object keyed by id (old format, backwards-compat).
...
// Accepts: undefined (writes current PROJECTS), an array, or an old-style object keyed by id.
function saveStoredProjects(val) {
  let arr = val;
  if (arr == null) arr = PROJECTS;
...
function applyStoredOverrides() {
  ...
  if (Array.isArray(stored)) {
    // New format — replace PROJECTS entirely so deletes + creates persist
    PROJECTS.length = 0;
    stored.forEach((p) => PROJECTS.push(p));
  } else if (typeof stored === "object") {
    // Old format — merge per-id overrides, append unknown ids
    Object.entries(stored).forEach(([id, ov]) => {
      const idx = PROJECTS.findIndex((p) => p.id === id);
      if (idx >= 0) Object.assign(PROJECTS[idx], ov);
      else PROJECTS.push(ov);
    });
  }
}
applyStoredOverrides();

Object.assign(window, { DISCIPLINES, PROJECTS, MARQUEE_TERMS, loadStoredProjects, saveStoredProjects, applyStoredOverrides });
