...
import { getProjects, saveStoredProjects } from './data';
...
export default function App() {
  ...
  const saveProject = useCallback((edited) => {
    setProjects((prev) => {
      const next = prev.map((p) => p.id === edited.id ? edited : p);
  ...
  const createProject = useCallback(() => {
    ...
    setOpenIdx(projects.length); // will be last after state update
  }, [projects]);

  const deleteProject = useCallback((proj) => {
    if (!proj) return;
  ...
  return (
    <>
      ...
      <Index projects={projects} onOpen={openProject} density={t.density} onCreate={createProject} />
      <About />
      <Footer />
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <span>
          <span style={{ display:"inline-block", width:8, height:8, background:"#ff3b1f", borderRadius:"50%", marginRight:8, verticalAlign:"middle" }} />
          REC · 23.976 · A.CAM
        </span>
