...
import { getProjects, saveStoredProjects, DISCIPLINES } from '../data';
import './admin.css';
...
export default function Admin() {
  const [projects, setProjects] = useState(() => clone(getProjects()));
  const [originals, setOriginals] = useState(() => clone(getProjects()));
  const [selectedId, setSelectedId] = useState(() => getProjects()[0]?.id);
  ...
  const addProject = () => {
    ...
    setSelectedId(id);
    showToast("New project created — remember to Save");
  };
      ...
      <main className="main">
        ...
        <div className="savebar">
          ...
          <div style={{ display: "flex", gap: 10 }}>
            ...
            <button className="btn" onClick={revertSelected} disabled={!isDirty(selectedId)}>
              Revert
            </button>
            <button className="btn primary" onClick={save} disabled={!anyDirty}>
              Save ⌘S
            </button>
          </div>
        </div>
    ...
    <>
      ...
      <div className="card">
        <div className="card-head">
          ...
          <span className="card-hint">6 placeholder colour pairs — drive the case study grid + hover scrub</span>
        </div>
        {p.palette.map((f, i) => (
      ...
      {/* ── Sections (Troubadour / multi-medium projects) ── */}
          ...
          {s.items && s.items.length > 0 && (
            <div style={{ marginTop: 18 }}>
              <div className="sect-items-head">
                <span>C1</span><span>C2</span><span>Title</span><span>Label / code</span><span />
              </div>
              {s.items.map((it, ii) => (
                <div key={ii} className="sect-item-row">
                  ...
                  <input type="text" value={it.lbl ?? ""} placeholder="Label / code" onChange={(e) => patchSectionItem(si, ii, "lbl", e.target.value)} />
